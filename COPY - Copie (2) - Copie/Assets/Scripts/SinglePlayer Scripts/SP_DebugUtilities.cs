...
public static class SP_DebugUtilities
{
    ...
    {
        string lala =
                        hitGo.GetComponent<SP_Shape>().Row + "-"
                        + hitGo.GetComponent<SP_Shape>().Column + "-"
                         + hitGo2.GetComponent<SP_Shape>().Row + "-"
                         + hitGo2.GetComponent<SP_Shape>().Column;
        Debug.Log(lala);

    }

    public static void ShowArray(SP_ShapesArray shapes)
    {
    ...
    /// <returns></returns>
    public static string GetArrayContents(SP_ShapesArray shapes)
    {
        ...
        for (int row = Constants.Rows - 1; row >= 0; row--)
        {
            ...
            for (int column = 0; column < Constants.Columns; column++)
            {
                ...
                {
                    var shape = shapes[row, column].GetComponent<SP_Shape>();
