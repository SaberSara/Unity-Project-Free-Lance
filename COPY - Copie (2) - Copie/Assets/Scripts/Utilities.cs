...
public static class Utilities
{
	...
	public static bool AreVerticalNeighbors(Shape s1 , Shape s2) {
		return (s1.Column == s2.Column);
	}

	public static bool AreHorizontalNeighbors(Shape s1 , Shape s2) {
    ...
    public static IEnumerable<GameObject> GetPotentialMatches(ShapesArray shapes)
    {
        ...
        for (int row = 0; row < Constants.Rows; row++)
        {
            for (int column = 0; column < Constants.Columns && 
                shapes[row, column].GetComponent<Shape>().Type!= "Stone_brick"; column++)
            {
